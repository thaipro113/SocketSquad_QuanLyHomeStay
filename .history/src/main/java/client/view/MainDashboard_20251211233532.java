...
public class MainDashboard extends JFrame {
    ...
    private void checkoutRoom() {
        ...
        // Fetch Services
        java.util.Map<String, Double> services = controller.getServices();
        double elecPrice = services != null ? services.getOrDefault("Electricity", 3500.0) : 3500.0;
        double waterPrice = services != null ? services.getOrDefault("Water", 15000.0) : 15000.0;
        double internetPrice = services != null ? services.getOrDefault("Internet", 100000.0) : 100000.0;

        // Calculate Duration
        long checkinTime = (currentTenant != null && currentTenant.getCheckinDate() != null)
                ? currentTenant.getCheckinDate().getTime()
                : System.currentTimeMillis(); // Fallback if null
        long now = System.currentTimeMillis();
        long diffMillis = now - checkinTime;
        long diffDays = diffMillis / (24 * 60 * 60 * 1000);

        // Logic tính tiền phòng:
        // Nếu < 1 ngày -> tính 1 ngày hoặc tính theo giờ (đơn giản là tính tròn 1 ngày
        // nếu ít hơn)
        // Hoặc tính theo giờ nếu muốn. Khách yêu cầu: "tính tiền phòng theo ngày , giờ"
        // Demo: Nếu < 1 ngày thì tính theo giờ (giả sử giá phòng là giá ngày).
        // Giá giờ = Giá ngày / 24.
        double roomPrice = currentRoom.getPrice();
        double roomFee = 0;
        String durationStr = "";
        ...
        if (diffDays < 1) {
            ...
            if (diffHours < 1)
                diffHours = 1; // Tối thiểu 1h
            double hourlyPrice = roomPrice / 24.0;
            roomFee = diffHours * hourlyPrice;
            durationStr = diffHours + " giờ";
        } else {
            roomFee = diffDays * roomPrice;
        ...
        panel.add(new JLabel(currentTenant != null ? currentTenant.getName() : "N/A"));

        panel.add(new JLabel("Thời gian thuê:"));
        panel.add(new JLabel(durationStr));

        panel.add(new JLabel("Tiền phòng (" + String.format("%,.0f", roomPrice) + "/ngày):"));
        JLabel lblRoomFee = new JLabel(String.format("%,.0f VNĐ", roomFee));
        panel.add(lblRoomFee);

        // Electricity
        panel.add(new JLabel("Chỉ số điện cũ:"));
        panel.add(new JLabel(String.valueOf(currentRoom.getElectricityIndex())));

        panel.add(new JLabel("Chỉ số điện mới:"));
        JTextField txtNewElec = new JTextField();
        panel.add(txtNewElec);

        panel.add(new JLabel("Tiền điện (" + String.format("%,.0f", elecPrice) + "/kWh):"));
        JLabel lblElecFee = new JLabel("0 VNĐ");
        panel.add(lblElecFee);

        // Water
        panel.add(new JLabel("Nước (m3):"));
        JTextField txtWaterUsage = new JTextField();
        panel.add(txtWaterUsage);

        panel.add(new JLabel("Tiền nước (" + String.format("%,.0f", waterPrice) + "/m3):"));
        JLabel lblWaterFee = new JLabel("0 VNĐ");
        panel.add(lblWaterFee);

        // Internet
        panel.add(new JLabel("Internet:"));
        JLabel lblInternetFee = new JLabel(String.format("%,.0f VNĐ", internetPrice));
        panel.add(lblInternetFee);

        // TOTAL
        panel.add(new JLabel("TỔNG CỘNG:"));
        JLabel lblTotal = new JLabel(String.format("%,.0f VNĐ", roomFee + internetPrice));
        ...
        Runnable updateCalc = () -> {
            try {
                int oldElec = currentRoom.getElectricityIndex();
                String newElecStr = txtNewElec.getText().trim();
                int newElec = newElecStr.isEmpty() ? oldElec : Integer.parseInt(newElecStr.replaceAll("[^0-9]", ""));

                int elecUsage = newElec - oldElec;
                if (elecUsage < 0)
                    elecUsage = 0;
                double elecFee = elecUsage * elecPrice;
                lblElecFee.setText(String.format("%,.0f VNĐ", elecFee));

                String waterStr = txtWaterUsage.getText().trim();
                int waterUse = waterStr.isEmpty() ? 0 : Integer.parseInt(waterStr.replaceAll("[^0-9]", ""));
                double waterFee = waterUse * waterPrice;
                lblWaterFee.setText(String.format("%,.0f VNĐ", waterFee));

                // Recalculate Total
                // Using final variables for roomFee and internetPrice inside lambda? No need if
                // valid scope.
                // roomFee is local variable, need to be effectively final. It is assigned once
                // in if/else block.
                // But compiler might complain if not effectively final. Let's strictly declare
                // final copy.
            } catch (Exception ex) {
                // Ignore parse errors
            }
        ...
        // Effective final wrapper
        final double finalRoomFee = roomFee;
        final double finalInternetFee = internetPrice;
        final Room finalCurrentRoom = currentRoom;
        ...
        javax.swing.event.DocumentListener dl = new javax.swing.event.DocumentListener() {
            ...
            void calc() {
                try {
                    int oldElec = finalCurrentRoom.getElectricityIndex();
                    String newElecStr = txtNewElec.getText().trim();
                    int newElec = newElecStr.isEmpty() ? oldElec
                            : Integer.parseInt(newElecStr.replaceAll("[^0-9]", ""));
                    ...
                    double total = finalRoomFee + finalInternetFee + elecFee + waterFee;
        ...
        txtNewElec.getDocument().addDocumentListener(dl);
        txtWaterUsage.getDocument().addDocumentListener(dl);

        int confirm = JOptionPane.showConfirmDialog(this, panel, "Thanh Toán & Trả Phòng", JOptionPane.OK_CANCEL_OPTION,
            ...
            try {
                ...
                if (newElec < oldElec) {
                    JOptionPane.showMessageDialog(this,
                            "Chỉ số điện mới không thể nhỏ hơn chỉ số cũ (" + oldElec + ")!", "Lỗi",
                ...
                int elecUsage = newElec - oldElec;
                double total = finalRoomFee + finalInternetFee + (elecUsage * elecPrice) + (waterUse * waterPrice);

                common.models.CheckoutPayload payload = new common.models.CheckoutPayload();
                payload.setRoomId(roomId);
                payload.setNewElectricityIndex(newElec);
                payload.setWaterUsage(waterUse);
                payload.setTotalAmount(total);

                if (controller.checkoutRoom(payload)) {
                    JOptionPane.showMessageDialog(this, "Trả phòng thành công!\nHóa đơn đã được tạo.", "Thành công",
                            JOptionPane.INFORMATION_MESSAGE);
                    loadRooms();
                    loadTenants();
                    loadHistory();
                    loadInvoices(); // Reload Invoice tab
                } else {
                    JOptionPane.showMessageDialog(this, "Trả phòng thất bại.", "Lỗi", JOptionPane.ERROR_MESSAGE);
                }
