    ...
    private void showAddTenantDialog() {
        ...
        if (option == JOptionPane.OK_OPTION) {
            try {
                ...
                if (targetRoom == null) {
                    JOptionPane.showMessageDialog(this, "Không tìm thấy phòng có ID: " + roomId);
                    return;
                }

                if (!"AVAILABLE".equals(targetRoom.getStatus())) {
                    JOptionPane.showMessageDialog(this,
                            "Phòng này hiện không trống (Trạng thái: " + targetRoom.getStatus() + ")");
                    return;
                }

                // Validate Unique ID Card and Phone
                ...
                if (existingTenants != null) {
                    for (Tenant t : existingTenants) {
                        if (t.getIdCard().equals(idCard)) {
                            JOptionPane.showMessageDialog(this, "Số CMND/CCCD đã tồn tại!");
                            return;
                        }
