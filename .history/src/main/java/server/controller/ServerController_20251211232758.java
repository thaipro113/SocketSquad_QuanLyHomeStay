    ...
    public Payload handleRequest(Payload request) {
        try {
            switch (request.getAction()) {
                ...
                case GET_SERVICES:
                    java.util.Map<String, Double> services = invoiceDAO.getAllServices();
                    return new Payload(Payload.Action.SUCCESS, services);

                case CHECKOUT_ROOM:
                    common.models.CheckoutPayload checkoutData = (common.models.CheckoutPayload) request.getData();
                    int checkoutRoomId = checkoutData.getRoomId();

                    // Lấy thông tin phòng hiện tại
                    Room roomToCheckout = null;
                    List<Room> allRoomsToCheck = roomDAO.getAllRooms();
                    for (Room r : allRoomsToCheck) {
                    ...
                    List<Tenant> allTenantsToCheck = tenantDAO.getAllTenants();
                    for (Tenant t : allTenantsToCheck) {
                    ...
                    // Calculate costs server-side for validation/record
                    double elecPrice = invoiceDAO.getServicePrice("Electricity");
                    double waterPrice = invoiceDAO.getServicePrice("Water");
                    double internetPrice = invoiceDAO.getServicePrice("Internet");

                    int oldElecIndex = roomToCheckout.getElectricityIndex();
                    int newElecIndex = checkoutData.getNewElectricityIndex();
                    int elecUsage = newElecIndex - oldElecIndex;
                    if (elecUsage < 0)
                        elecUsage = 0; // Prevent negative

                    int waterUsage = checkoutData.getWaterUsage();

                    // Calculate Room Fee (Duration)
                    double roomPrice = roomToCheckout.getPrice(); // Monthly price? Or daily? Assume Monthly for now but
                                                                  // usually generic price.
                    // Requirement says: "tính tiền phòng theo ngày , giờ tùy theo khách trả phòng"
                    // If price is monthly, we need logic. If price is daily, simple multiplication.
                    // Usually Rooms.price is displayed as Price. Let's assume Price is Daily or we
                    // calculate based on duration.
                    // Current Rooms.price logic isn't strictly defined but usually daily/monthly.
                    // Let's implement Day/Hour calculation:
                    // If < 1 day, calculate hourly? Or just day fraction?
                    // Let's rely on the passed TotalAmount for now or calculate standard.
                    // BETTER: Calculate based on Checkin Date.

                    long durationMillis = 0;
                    if (tenantToCheckout != null && tenantToCheckout.getCheckinDate() != null) {
                        durationMillis = System.currentTimeMillis() - tenantToCheckout.getCheckinDate().getTime();
                    }
                    // For now, accepting the TotalAmount from client or simple addition.
                    // To be safe, let's trust the calculation logic mirrors client side.
                    // But we will save the invoice.

                    Invoice newInvoice = new Invoice();
                    newInvoice.setRoomId(checkoutRoomId);
                    newInvoice.setMonth(java.time.LocalDate.now().getMonthValue());
                    newInvoice.setYear(java.time.LocalDate.now().getYear());
                    newInvoice.setElectricityUsage(elecUsage);
                    newInvoice.setWaterUsage(waterUsage);
                    newInvoice.setInternetFee(internetPrice);
                    newInvoice.setTotalAmount(checkoutData.getTotalAmount()); // Trusting client for complex room fee
                                                                              // logic for now, or re-calc
                    newInvoice.setStatus("PAID"); // Usually checkout implies payment
                    ...
                    // Cập nhật trạng thái phòng thành AVAILABLE và chỉ số điện nước mới
                    roomToCheckout.setStatus("AVAILABLE");
                    roomToCheckout.setElectricityIndex(newElecIndex);
                    // roomToCheckout.setWaterIndex(newWaterIndex); // If we tracked water index.
                    // User said "water usage", so maybe just usage.
                    // If user wants to update water index too, we need NewWaterIndex.
                    // The prompt said: "nước thì tính theo m3 lúc nhập số mới xong thì cập nhật số
                    // điện mới vào số điện mặc định để sử dụng tiếp"
                    // Only mentioned updating electricity index ("cập nhật số điện mới"). Water
                    // seems to be usage based or maybe index too?
                    // "nước thì tính theo m3" -> Usage.
                    // "cập nhật số điện mới vào số điện mặc định" -> Update Electricity Index.
