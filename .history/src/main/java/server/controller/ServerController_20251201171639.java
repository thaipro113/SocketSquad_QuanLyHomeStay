    ...
    public Payload handleRequest(Payload request) {
        try {
            switch (request.getAction()) {
                ...
                case CALCULATE_COST:
                    // Data: [RoomID, Month, Year, ElecUsage, WaterUsage]
                    // Ideally pass a DTO, but for simplicity we can pass an incomplete Invoice
                    // object
                ...
                case UPLOAD_FILE:
                    // Simple file upload handling (saving bytes to disk)
                    // This requires a custom object containing filename and byte[]
                    // For this example, we assume the client sends a custom FilePayload (not
                    // defined yet, let's define inline or handle simply)
                    // Implementation skipped for brevity, needs a specific DTO.
                    return new Payload(Payload.Action.FAILURE, null, "File upload not implemented yet");
