        ...
        try {
            switch (request.getAction()) {
                ...
                case CHECKOUT_ROOM: {
                    ...
                    newInvoice.setStatus("ĐÃ THANH TOÁN");
                    ...
                    if (roomDAO.updateRoom(roomToCheckout)) {
                        String timeStr = diffHours < 24 ? diffHours + " giờ"
                                : diffDays + " ngày (+ " + (diffHours % 24) + " giờ lẻ)";
                        String msg = String.format(
                                "Trả phòng thành công!\n" +
                                        ...
                                        "--------------------------------\n" +
                                        "TỔNG CỘNG: %,.0f VNĐ",
                                timeStr, roomCost, oldElec, newElec, electricityCost, oldWater, newWater, waterCost,
                                internetPrice, totalAmount);
                ...
                case CHECKIN_ROOM:
                    ...
                    if (roomDAO.updateRoom(roomToCheckin)) {
                        // Cập nhật ngày check-in của khách thuê thành NOW
                        List<Tenant> tenantList = tenantDAO.getAllTenants();
                        for (Tenant t : tenantList) {
                            if (t.getRoomId() == checkinRoomId) {
                                t.setCheckInDate(new java.util.Date());
